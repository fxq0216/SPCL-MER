...
class PointRecuder(nn.Module):#downsample
    """
...
def pairwise_cos_sim(x1: torch.Tensor, x2: torch.Tensor):#compute sim
    """
...
class Cluster(nn.Module):
    def __init__(self, dim, out_dim, proposal_w=2, proposal_h=2, fold_w=2, fold_h=2, heads=4, head_dim=24,
        ...
        self.GCBlock = GlobalContextBlock(in_channels=dim)#gc block

        self.conv_weight = nn.Conv2d(out_dim, 1, 1)
        self.conv_space = nn.AvgPool2d(7, stride=7)
        self.up = nn.UpsamplingNearest2d(scale_factor=7)

        # self.fc64 = nn.Linear(64, 64)
        # self.fc128 = nn.Linear(16, 16)
        # self.fc320 = nn.Linear(4, 4)
        
        self.fc64 = nn.Sequential(
            nn.Linear(64, 32, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(32, 64, bias=False),
            # nn.Sigmoid()
        )
        self.fc128 = nn.Sequential(
            nn.Linear(16, 8, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(8, 16, bias=False),
            # nn.Sigmoid()
        )
        self.fc320 = self.fc128 = nn.Sequential(
    ...
    def forward(self, x):  # [b,c,w,h]
        ...
        mask.scatter_(1, sim_max_idx, 1.)#确定每个点属于那一个中心
        sim = sim * mask#其他位置置0
        # print(sim.shape)
        ...
        if self.return_center:
            ...
            out = rearrange(out, "b (w h) c -> b c w h", w=w)#这是公式2，FC括号内的东西

        # print(out.shape)
        # a, b, c, d = out.shape
        # i,m,n=mask.shape
        # out_weight = rearrange(out, "b c w h -> b c (w h)")
        # mask_weight = mask.unsqueeze(dim=1)
        # out_weight = out_weight.unsqueeze(dim=2)
        # mask_weight = mask_weight.repeat(1,b,1,1)
        # out_weight = out_weight.repeat(1,1,m,1)
        # center_weight = mask_weight*out_weight
        # print(center_weight.shape)

        # # out = 
        # out_weight = rearrange(out, "(b f1 f2) c w h -> b c f1 f2 (w h)", f1=self.fold_w, f2=self.fold_h)
        # out_weight = torch.mean(out_weight, dim=4)
        # # out_weight = self.conv_weight(out_weight)
        ...
        #压缩通道
        _, ch, _, _ = out.shape
        out_weight = self.conv_weight(out).squeeze(dim=1)
        out_weight = self.conv_space(out_weight)
        
        _, wid, hei = out_weight.shape
        out_weight = rearrange(out_weight, "b w h -> b (w h)")
        b, c = out_weight.shape
        if c==64:
            out_weight = self.fc64(out_weight)
            out_weight = torch.sigmoid(out_weight)
            out_weight = rearrange(out_weight, "b (w h) -> b w h", w = wid)
            out_weight = out_weight.unsqueeze(dim=1)
            out_weight = self.up(out_weight)
            out_weight = out_weight.expand_as(out)
            out = out*out_weight
            # print(out_weight.shape)
        if c==128:
            out_weight = self.fc128(out_weight)
            out_weight = torch.sigmoid(out_weight)
            out_weight = rearrange(out_weight, "b (w h) -> b w h", w = wid)
            out_weight = out_weight.unsqueeze(dim=1)
            out_weight = self.up(out_weight)
            ...
            out = out*out_weight
        if c==320:
            out_weight = self.fc320(out_weight)
            out_weight = torch.sigmoid(out_weight)
            out_weight = rearrange(out_weight, "b (w h) -> b w h", w = wid)
            out_weight = out_weight.unsqueeze(dim=1)
            out_weight = self.up(out_weight)
...
class ContextCluster(nn.Module):
    ...
    def __init__(self, layers, embed_dims=None,
        ...
        self.mean = nn.Conv2d(512, 512, 7, stride=1, padding=0, groups=512)
    ...
    def forward(self, x):
        ...
        if self.fork_feat:
            ...
            return x
        x = self.norm(x)#b,c,7,7
        # print(x.shape)


        
        #替代平均池化
        x = torch.squeeze((self.mean(x))) 
...
class Cocatt(nn.Module):
    """backbone + projection head"""
    def __init__(self, head='mlp', feat_dim=128, num_class=5):
        super(Cocatt, self).__init__()
        self.encoder = coc_small()
        # self.encoder = coc_medium()
        dim_in = 512
        if head == 'linear':
    ...
    def forward(self, x):
        feat = self.encoder(x)#用一层全连接进行投影
...
class LinearClassifier(nn.Module):
    ...
    def __init__(self, name='resnet18', num_classes=5):
        ...
        # self.fc = nn.Linear(feat_dim, feat_dim//2)
        # self.relu1 = nn.ReLU(inplace=True)
        self.fc1 = nn.Linear(feat_dim, num_classes)#casme只有这一层投影
...
if __name__ == '__main__':
    ...
    model = coc_small()
    # model = SupConcluster().cuda()
    # model.eval()
    out = model(input)
    # out1 = model(input)
    # print(out.equal(out1))
    # print(model)
    print(out.shape)
